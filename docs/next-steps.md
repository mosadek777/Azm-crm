...
| `004 FR-005` — notify ahead of **each SLA threshold** (the second producer; the task producer IS built) | The thresholds are `005 FR-006`, and `005` is blocked on `[CLARIFY-1]`. A reminder fired against an invented target trains an agent to act on a deadline nobody agreed. See `decisions-pending.md` §20, board card `reminder-sla-thresholds` | Hours, not days — `dueReminders` is already a pure function and the engine becomes a second caller into the same in-app surface |
| `004 FR-013` — deliver notifications **by email and by push** (in-app IS built) | Spec `003` does not exist: no transport, no provider, no template store, no push-token store. Same category as `R5` in `remaining.md` — nothing was shortcut, because nothing could be built. Board card `reminder-channels-email-push` | Blocked on spec `003` — not estimable until a provider is named |
| A **scheduler** — anything that runs when nobody has the product open | There is no cron, queue, worker or timer in this project. `005 §11` already specifies one ("run scheduled automation (internal) … idempotent per ticket per occurrence"), so building a second inside `004` would pre-empt a design `005` owns. Until it exists, reminders are evaluated on request and the workspace says so in words. Board card `reminder-scheduler` | Belongs to the `005` engine, not to `004` |
